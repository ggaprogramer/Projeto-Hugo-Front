...
import AgendamentoConfirmadoComponent from './components/AgendamentoConfirmadoComponent';

export default async function AgendamentoConfirmado() {
    ...
    if(userIsAuthenticated?.token && userIsAuthenticated.roles?.length !== 0 && userIsAuthenticated.roles?.indexOf('PROFILE') !== -1){
        return (
            <>
                <AgendamentoConfirmadoComponent authToken={userIsAuthenticated?.token}/>
            </>
        );
    } else {
        redirect('/auth/login');
