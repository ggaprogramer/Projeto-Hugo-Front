...
import '../styles/agendamentos-profissional.scss';
...
import {ControlSessionFilterProfessional, SessionFilterProfessional, SessionProfessional} from '../interfaces';
import {Errors} from '@auth/interfaces';
import ErrorAuth from '@auth/components/ErrorAuth'
import getSessionProfessional from '../functions/getSessionProfessional';
import Pagination from '../components/Pagination';
import { GoChevronDown } from "react-icons/go";
import { GoChevronUp } from "react-icons/go";

export default function AgendamentosProfissional(){
    const [control, setControl] = useState<ControlSessionFilterProfessional>({
    ...
    const [proximaConsulta, setProximaConsulta] = useState<SessionProfessional | null>(null);
    useEffect(() => {
        if(control?.sessionsProfessional && control?.sessionsProfessional?.length > 0){
            for(let i = 0; i < control?.sessionsProfessional?.length; i++){
                if(control?.sessionsProfessional[i]?.status === 'APPROVED'){
                    setProximaConsulta(control.sessionsProfessional[i]);
                    break;
                }
            }
        };
    }, [control.sessionsProfessional]);
    ...
    const makeFilter = async () => {
        const body: SessionFilterProfessional = {
        ...
        try{
            viewLoader();
            const json = await getSessionProfessional(body);
            const data: SessionProfessional[] = json.content; 
            setControl({...control, 
                sessionsProfessional: data, 
    ...
    const percentProgress = (name: string) => {
        if(control && control?.sessionsProfessional && control?.sessionsProfessional.length > 0){
            let count = 0;
            for(let i = 0; i <= control.sessionsProfessional.length; i++){
                if(name === 'PROCESSING' && control?.sessionsProfessional[i]?.status === 'PROCESSING'){
                    count+=1;
                } else if(name === 'APPROVED' && control?.sessionsProfessional[i]?.status === 'APPROVED'){
                    count+=1;
                } else if(name === 'FINISHED' && control?.sessionsProfessional[i]?.status === 'FINISHED'){
                    count+=1;
                } else if(name === 'CANCELLED' && control?.sessionsProfessional[i]?.status === 'CANCELLED'){
                    count+=1;
                }
            }
            if(control && control?.sessionsProfessional && control?.sessionsProfessional.length > 0) {
                return `${count / control.sessionsProfessional.length * 100}%`;
            } 
            return '0%';
        } 
        return '0%';
                ...
                <div className='metricas'>
                    {proximaConsulta &&
                        <div className='proxima-consultas'>
                            ...
                            <div>
                                {
                                    proximaConsulta.linkPhotoProfile
                                    ?
                                    <img src={proximaConsulta.linkPhotoProfile} alt="" />
                                    :
                                    <img src="/user.png" alt="" />
                                }
                                <div>
                                    <h4>
                                        {proximaConsulta.gender === 'MULHER' ? 'Dra. ' : 'Dr. '}  {proximaConsulta.profileName}
                                    </h4>
                                    <p>
                                        <FaClock/>
                ...
                <div className='agendamentos'>
                    ...
                    <div className='tabela'>
                        <table>
                            ...
                            <tbody>
                                {control.sessionsProfessional.map(session => {
                                    return (
                                        <tr key={session.id}>
                                            <td className='terapeuta'>
                                                {
                                                    session.linkPhotoProfile
                                                    ?
                                                    <img src={session.linkPhotoProfile} alt="" />
                                                    :
                                                    <img src="/user.png" alt="" />
                                                }
                                                <div>
